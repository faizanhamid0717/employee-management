...
interface EmployeeFormProps {
  ...
  onSave: (
    employee: Omit<Employee, "id" | "createdAt"> & { id?: string }
  ) => void;
  onCancel: () => void;
}
...
const EmployeeForm: React.FC<EmployeeFormProps> = ({
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Check file size (limit to 100KB to prevent localStorage issues)
      const maxSize = 100 * 1024; // 100KB in bytes
      if (file.size > maxSize) {
        setErrors((prev) => ({
          ...prev,
          profileImage:
            "Image too large. Please use an image under 100KB or provide an image URL instead.",
        }));
        return;
      }
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-xs text-gray-500 text-center">
            Upload image (max 100KB) or use URL below
          </p>
          {errors.profileImage && (
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-500">
            Recommended: Use image URLs to avoid storage limits
          </p>
        </div>
